...
describe('MessageClassifier', () => {
  describe('classifyMessage', () => {
    test('자막 편집 명령어를 올바르게 분류', () => {
      ...
      testCases.forEach(({ message, expected }) => {
        ...
        if (expected.timeAdjustment) {
          expect(result.extractedDetails?.timeAdjustment).toBe(expected.timeAdjustment)
        }
        
        if (expected.styleProperty) {
          expect(result.extractedDetails?.styleProperty).toBe(expected.styleProperty)
        }
        
        if (expected.animationType) {
          expect(result.extractedDetails?.animationType).toBe(expected.animationType)
        }
      })
    })
    ...
    test('신뢰도 점수가 적절히 계산됨', () => {
      ...
      const highResult = MessageClassifier.classifyMessage(highConfidenceMessage)
...
export function runMessageClassifierTests() {
  ...
  testMessages.forEach(message => {
    const result = MessageClassifier.classifyMessage(message)
    console.log(`\n메시지: "${message}"`)
    console.log(`분류: ${result.actionType} (신뢰도: ${result.confidence.toFixed(2)})`)
