...
export const useYouTubeUpload = (): UseYouTubeUploadReturn => {
  ...
  const serviceRef = useRef<{ abortUpload: (id: string) => boolean } | null>(null)
  ...
  const uploadVideo = useCallback(async (data: YouTubeUploadData): Promise<YouTubeUploadResponse> => {
    if (isUploading) {
      return {
        success: false,
