...
export function normalizeSpeakerList(speakers: string[]): {
  ...
  const uniqueSpeakers = Array.from(new Set(
    speakers.filter(speaker => speaker && speaker.trim() !== '')
  ))
 ...
 */
export function analyzeSpeakerUsage(clips: ClipItem[]): Record<string, {
  clipCount: number
  wordCount: number
 ...
 * 화자 이름의 유효성을 검사합니다
 */
export function validateSpeakerName(name: string, existingSpeakers: string[]): {
  isValid: boolean
  error?: string
} {
