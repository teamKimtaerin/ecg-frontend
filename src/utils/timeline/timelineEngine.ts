...
export class TimelineEngine {
  ...
  initializeFromClips(originalClips: ClipItem[]): TimelineClip[] {
    ...
    for (const [index, originalClip] of originalClips.entries()) {
      // 클립의 실제 시작/끝 시간 계산
      const startTime = originalClip.words.length > 0 ? originalClip.words[0].start : currentTime
      const endTime = originalClip.words.length > 0 
   ...
   */
  mapTimelineToSource(timelineClip: TimelineClip, timelineTime: number): {
    sourceClipId: string
    sourceTime: number
    isValid: boolean
  } {
    // 타임라인 시간이 클립 범위 내에 있는지 확인
    if (timelineTime < timelineClip.startTime || 
    ...
    return {
      ...
      isValid: sourceTime >= timelineClip.inPoint && sourceTime <= timelineClip.outPoint
    }
  }
   ...
   */
  getActiveClipsAtTime(clips: TimelineClip[], time: number, track?: TrackType): TimelineClip[] {
    return clips.filter(clip => {
      const isInTimeRange = time >= clip.startTime && time < clip.startTime + clip.duration
   ...
   * 클립을 분할
   */
  splitClip(clip: TimelineClip, splitTime: number): [TimelineClip, TimelineClip] | null {
  ...
  trimClip(
    ...
    const inPoint = newInPoint !== undefined ? 
      Math.max(0, newInPoint) : clip.inPoint
    const outPoint = newOutPoint !== undefined ? 
      Math.max(inPoint + 0.1, newOutPoint) : clip.outPoint
   ...
   * 클립들 간의 겹침 검사
   */
  findOverlappingClips(clips: TimelineClip[], trackType?: TrackType): TimelineClip[][] {
    const overlapping: TimelineClip[][] = []
    
    // 같은 트랙의 클립들만 비교
    const trackedClips = trackType ? 
      clips.filter(clip => clip.track === trackType) : clips
  ...
  insertEdit(
    clips: TimelineClip[], 
    insertClip: TimelineClip
  ): TimelineClip[] {
    const existingClips = clips.filter(c => 
      c.trackIndex === insertClip.trackIndex && 
      c.track === insertClip.track
    )

    // 삽입 지점 이후의 클립들을 삽입될 클립의 길이만큼 뒤로 이동
    const adjustedClips = clips.map(clip => {
      if (clip.trackIndex === insertClip.trackIndex && 
  ...
  overwriteEdit(
    ...
    const processedClips = clips.reduce<TimelineClip[]>((acc, clip) => {
      // 다른 트랙이면 그대로 추가
      if (clip.track !== newClip.track || clip.trackIndex !== newClip.trackIndex) {
        acc.push(clip)
        return acc
      }
      ...
      } else if (clipStart >= newStart && clipStart < newEnd && clipEnd > newEnd) {
        // 뒷부분만 남김
