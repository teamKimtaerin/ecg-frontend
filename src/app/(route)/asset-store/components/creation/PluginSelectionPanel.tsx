...
import { LuFlame, LuMove, LuRotateCw, LuSparkles, LuStar, LuTrendingUp, LuType, LuWaves, LuZap } from 'react-icons/lu'
...
export const PluginSelectionPanel: React.FC<PluginSelectionPanelProps> = ({
  ...
  const plugins = availableAssets.length > 0 ? availableAssets.slice(0, 9) : defaultPlugins
  ...
  return (
    <div className={clsx('flex flex-col items-center justify-center h-full w-full p-4', className)}>
      <div className="text-center mb-4">
        <h2 className="text-lg font-bold text-gray-800 mb-1">기반 플러그인 선택</h2>
      </div>

      <div className="grid grid-cols-3 gap-2 w-full max-w-md">
        {plugins.map((plugin) => {
          const IconComponent = availableAssets.length > 0 ? LuSparkles : (plugin as any).icon || LuSparkles
          const gradientColor = availableAssets.length > 0 ? 'from-gray-500 to-gray-600' : (plugin as any).color || 'from-gray-500 to-gray-600'
          ...
          return (
            ...
            >
              ...
              <h3 className="font-medium text-gray-800 text-xs leading-tight">{plugin.title}</h3>
              <p className="text-xs text-gray-500 leading-none mt-0.5">{plugin.description}</p>
            </button>
          )
        })}
      </div>
