...
import { RenderStatus, RenderOptions, RendererScenario } from '@/services/api/types/render.types'
...
export function useServerVideoExport(): UseServerVideoExportResult {
   ...
   * 진행 상황 업데이트 콜백
   */
  const handleProgress = useCallback((status: RenderStatus) => {
    ...
    if (status.progress && status.progress > 0) {
      ...
      timeRemaining = Math.max(0, Math.round(totalEstimatedTime - elapsedTime))
    ...
    try {
      ...
      if ('showSaveFilePicker' in window && window.showSaveFilePicker) {
        try {
          ...
          showToast('저장 위치 선택 실패, 기본 다운로드로 진행합니다', 'warning')
        }
      }
      ...
      if (!createResponse.success || !createResponse.data) {
        throw new Error(createResponse.error?.message || '렌더링 작업 생성 실패')
      }
      ...
      console.log(`🚀 GPU 렌더링 작업 시작: ${job.jobId} (예상: ${job.estimatedTime}초)`)
      ...
      if (errorMessage.includes('저장 위치 선택이 취소')) {
        ...
        userMessage = '비디오 파일이나 자막 데이터에 문제가 있습니다. 파일을 확인해주세요.'
      } else if (errorMessage.includes('GPU')) {
        userMessage = 'GPU 서버에 일시적인 문제가 발생했습니다. 잠시 후 다시 시도해주세요.'
   ...
   */
  useEffect(() => {
    if (state.status === 'completed' && state.downloadUrl && state.selectedFileHandle) {
      // 자동 저장 실행
