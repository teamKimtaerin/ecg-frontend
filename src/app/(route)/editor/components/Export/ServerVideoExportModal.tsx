...
export default function ServerVideoExportModal({
  ...
  const [phase, setPhase] = useState<'ready' | 'exporting' | 'completed' | 'error'>('ready')
  ...
  const handleStartExport = async () => {
    ...
    try {
      ...
      await startExport(videoUrl, scenario, {
        width: 1920,
        height: 1080,
        fps: 30,
        quality: 90,
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} closeOnBackdropClick={!isExporting} isblind={false}>
        <div className="p-6">
          {/* 준비 단계 */}
          {phase === 'ready' && (
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-sm text-gray-600">
                  <span className="font-semibold">영상 정보:</span><br />
          ...
          {phase === 'completed' && downloadUrl && (
            <div className="space-y-4">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">
                  {selectedFileHandle ? '선택한 위치에 자동으로 저장되었습니다' : '고품질 영상이 준비되었습니다'}
                </p>
              </div>
