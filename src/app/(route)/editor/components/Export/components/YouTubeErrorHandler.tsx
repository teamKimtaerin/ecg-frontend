...
export default function YouTubeErrorHandler({
    ...
    switch (errorType) {
      case 'login':
        return {
          ...
          solution: '업로드를 시작하면 자동으로 YouTube 로그인 페이지가 열립니다.',
      ...
      case 'browser':
        return {
          ...
          solution: '터미널에서 "npx playwright install chromium" 명령어를 실행해주세요.',
          actionText: '설치 후 재시도'
        }
      ...
      default:
        return {
          ...
          solution: '문제가 지속되면 브라우저를 새로고침하거나 지원팀에 문의해주세요.',
          actionText: '재시도'
        }
    }
  }
  ...
  if (!showDetails) {
    return (
      <div className={`flex items-center gap-2 p-2 ${config.bgColor} ${config.borderColor} border rounded ${className}`}>
        {config.icon}
    ...
    <div className={`${config.bgColor} ${config.borderColor} border rounded-lg p-4 ${className}`}>
        ...
        <div className="flex gap-2">
          ...
          {errorType === 'browser' && (
            <button
              onClick={() => {
                // 터미널 명령어 복사
                navigator.clipboard?.writeText('npx playwright install chromium')
              }}
      ...
      {errorType === 'unknown' && (
        <div className="mt-3 pt-3 border-t border-current border-opacity-20">
          ...
          <ul className={`text-xs ${config.textColor} opacity-70 mt-1 space-y-1`}>
...
// 편의 컴포넌트들
export const YouTubeLoginError: React.FC<Omit<YouTubeErrorHandlerProps, 'errorType'>> = (props) => (
  <YouTubeErrorHandler {...props} errorType="login" />
)

export const YouTubeBrowserError: React.FC<Omit<YouTubeErrorHandlerProps, 'errorType'>> = (props) => (
  <YouTubeErrorHandler {...props} errorType="browser" />
)

export const YouTubeNetworkError: React.FC<Omit<YouTubeErrorHandlerProps, 'errorType'>> = (props) => (
  <YouTubeErrorHandler {...props} errorType="network" />
)

export const YouTubeUploadError: React.FC<Omit<YouTubeErrorHandlerProps, 'errorType'>> = (props) => (
  <YouTubeErrorHandler {...props} errorType="upload" />
)